...
public class ComputerBrain {
	...
	public void ThinkDamnYou(edu.ufp.inf.sd.rmi.project.client.awgame.players.Base ply) {
		...
		if (!DoneUnits) {
			...
			edu.ufp.inf.sd.rmi.project.client.awgame.buildings.Base bld = Game.builds.get(CityCount);
			CityCount++;
	...
	private void HandleBuilding(edu.ufp.inf.sd.rmi.project.client.awgame.buildings.Base bld) {
		if (!bld.Menu.equals("")) {
			if (!bld.Locked) {
	...
	private boolean BuildingInRange(Base unit) {
		for (int i = 0; i < unit.map.size(); i++) {
			for (edu.ufp.inf.sd.rmi.project.client.awgame.buildings.Base bld : Game.builds) {
