...
import edu.ufp.inf.sd.rmi.project.client.awgame.buildings.*;
import edu.ufp.inf.sd.rmi.project.client.awgame.buildings.Base;
import edu.ufp.inf.sd.rmi.project.client.awgame.players.Andy;
import edu.ufp.inf.sd.rmi.project.client.awgame.players.Colin;
import edu.ufp.inf.sd.rmi.project.client.awgame.players.Max;
import edu.ufp.inf.sd.rmi.project.client.awgame.players.Sammi;
import edu.ufp.inf.sd.rmi.project.client.awgame.units.*;
...
public class ListData {
	 ...
	 */
	public edu.ufp.inf.sd.rmi.project.client.awgame.players.Base CreateCommander(int which, int team, int money, boolean npc) {
		switch (which) {
	 ...
	 */
	public edu.ufp.inf.sd.rmi.project.client.awgame.units.Base CreateUnit(int type, int owner, int xx, int yy, boolean active) {
		switch (type) {
	...
	public void LoadCommander(String Name, boolean Defeated, int Team, int Money, int Kills, int Loses, int Power, int Using, boolean npc) {
		for (int i = 0; i < Game.displayC.size(); i++) {
			if (Game.displayC.get(i).name.equals(Name)) {
				...
				edu.ufp.inf.sd.rmi.project.client.awgame.players.Base ply = Game.player.get(Game.player.size()-1);
	...
	public void LoadUnit(String Name, int Owner, int Health, int Ammo, int Fuel, int X, int Y, boolean Acted) {
		for (int i = 0; i < Game.displayU.size(); i++) {
			if (Game.displayU.get(i).name.equals(Name)) {
				...
				edu.ufp.inf.sd.rmi.project.client.awgame.units.Base unit = Game.units.get(Game.units.size()-1);
				unit.health = Health;
				unit.Ammo = Ammo;
				unit.Fuel = Fuel;
			}
