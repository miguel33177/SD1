...
import edu.ufp.inf.sd.rabbitmqservices.project.consumer.awgame.menus.EditorMenu;
import edu.ufp.inf.sd.rabbitmqservices.project.consumer.awgame.menus.Pause;
import edu.ufp.inf.sd.rabbitmqservices.project.consumer.awgame.menus.StartMenu;
...
public class InputHandler implements KeyListener,MouseListener,ActionListener {
	...
	public void keyPressed(KeyEvent e){
		...
		if (i==exit) {System.exit(0);}
			...
			try {
				if (i == up) {
					String action = "up," + Game.id;
					Game.o.channel.basicPublish("", "queue_w", null, action.getBytes(StandardCharsets.UTF_8));
				} else if (i == down) {
					String action = "down," + Game.id;
					Game.o.channel.basicPublish("", "queue_w", null, action.getBytes(StandardCharsets.UTF_8));
				} else if (i == left) {
					String action = "left," + Game.id;
					Game.o.channel.basicPublish("", "queue_w", null, action.getBytes(StandardCharsets.UTF_8));
				} else if (i == right) {
					String action = "right," + Game.id;
					Game.o.channel.basicPublish("", "queue_w", null, action.getBytes(StandardCharsets.UTF_8));
				} else if (i == select) {
					String action = "select," + Game.id;
					Game.o.channel.basicPublish("", "queue_w", null, action.getBytes(StandardCharsets.UTF_8));
				} else if (i == cancel) {
					String action = "cancel," + Game.id;
					Game.o.channel.basicPublish("", "queue_w", null, action.getBytes(StandardCharsets.UTF_8));
				} else if (i == start) {
					String action = "start," + Game.id;
					Game.o.channel.basicPublish("", "queue_w", null, action.getBytes(StandardCharsets.UTF_8));
					new Pause();
				} else if (i == passTurn) {
					String action = "passTurn," + Game.id;
					Game.o.channel.basicPublish("", "queue_w", null, action.getBytes(StandardCharsets.UTF_8));
				}
			}catch (Exception x){
				x.printStackTrace();
