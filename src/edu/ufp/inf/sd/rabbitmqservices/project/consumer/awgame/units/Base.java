package edu.ufp.inf.sd.rabbitmqservices.project.consumer.awgame.units;
...
public class Base {
	...
	private void capture(int destx,int desty) {
		...
		if (destx==x&&desty==y) {
			if (bld!=-1) {
				edu.ufp.inf.sd.rabbitmqservices.project.consumer.awgame.buildings.Base city = Game.builds.get(bld);
