 ...
 */
public class Save {
	final String path = "edu/ufp/inf/sd/rabbitmqservices/project/consumer/awgame/saves/";
	...
	private String SavePlayerData() {
		...
		for (int i = 0; i < Game.btl.totalplayers; i++) {
	    	edu.ufp.inf.sd.rabbitmqservices.project.consumer.awgame.players.Base ply = Game.player.get(i);
	...
	private String SaveUnitData() {
	    ...
	    for (int i = 0; i < Game.units.size(); i++) {
	    	edu.ufp.inf.sd.rabbitmqservices.project.consumer.awgame.units.Base unit = Game.units.get(i);
